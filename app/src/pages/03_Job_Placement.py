...
with col1:
    ...
    response = requests.get('http://api:4000/api/positions')
...
try:
    ...
    if selected_status == 'UNDER REVIEW':
        response = requests.get('http://api:4000/api/applications/under_review')
        applications = response.json() if response.status_code == 200 else []
    elif selected_status != 'All Statuses':
        response = requests.get(f'http://api:4000/api/applications/{selected_status}')
        applications = response.json() if response.status_code == 200 else []
    else:
        # Get all applications for each position
        applications = []
        for position in positions:
            response = requests.get(f'http://api:4000/api/applications/{position["JobID"]}')
    ...
    if applications:
        ...
        if st.button("Update Selected to ACCEPTED"):
            ...
            for app_id in selected_rows:
                response = requests.post(
                    f'http://api:4000/api/applications/ACCEPTED',
        ...
        if st.button("Reject Selected Applications"):
            ...
            for app_id in selected_rows:
                response = requests.put(f'http://api:4000/api/applications/reject/{app_id}')
                if response.status_code == 200:

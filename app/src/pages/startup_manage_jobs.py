...
with tab1:
    ...
    if submit_button:
        ...
        if not job_title or not company_name or not location or not job_description:
            ...
            try:
                response = requests.post("http://localhost:4000/jobs", json=job_data)
...
with tab2:
    ...
    if action == "View Work Experiences":
        ...
        if st.button("Fetch Work Experiences"):
            try:
                response = requests.get(f"http://localhost:4000/workexperiences/{student_id}")
    ...
    if action == "Add Work Experience":
        ...
        if submit_button:
            ...
            try:
                response = requests.post("http://localhost:4000/workexperiences", json=work_data)
                if response.status_code == 201:
    ...
    if action == "Update Work Experience":
        ...
        if st.button("Update Work Experience"):
            ...
            try:
                response = requests.put(f"http://localhost:4000/workexperiences/{job_id}", json=work_data)
                if response.status_code == 200:
    ...
    if action == "Delete Work Experience":
        ...
        if st.button("Delete Work Experience"):
            try:
                response = requests.delete(f"http://localhost:4000/workexperiences/{job_id}")
                if response.status_code == 200:
    ...
    if action == "View Latest Work Experience":
        ...
        if st.button("Fetch Latest Experience"):
            try:
                response = requests.get(f"http://localhost:4000/workexperiences/{student_id}/latest")
                if response.status_code == 200:
                    data = response.json()
                    st.json(data)
                else:
